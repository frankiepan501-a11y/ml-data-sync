...
本土店月度补数只处理当前活跃的巴西店 `2378517428` 和本土 3 店 `3383185411`。系统先按较宽的 `date_created` 窗口拉齐订单，再分别按 `America/Sao_Paulo` / `America/Mexico_City` 时区的 `date_closed` 归入自然月。只有搜索结果、订单明细、目标月范围与物流费用全部完整，且账单没有未识别费用时，才进入飞书写入。

### 2026-09-08 本土店自然月与账单费用修复

- 问题：订单按创建时间归月，导致平台当地 9 月确认的销售进入 8 月；Display Ads、Full 仓储/违规、退货处理费没有完整进入毛利。
- 修复：改为“宽窗拉取 + 当地 `date_closed` 二次筛选”；销售日期也使用当地确认日期。账单同时读取 ML 通用明细和 Full 专用明细，按 `detail_id` 去重；Product Ads 继续采用投放发生月报表，不重复扣账单金额。
- 安全门禁：物流费字段缺失不能当 0；未知账单类型会停止写入；月度重算会保留已有头程/海外仓成本并回读校验。当前周期已有数据时，必须传 `preserve_existing_as=month_YYYY-MM_<历史标签>`，系统才会把旧行整体归档并切换修正版；没有归档标签则停止写入。
- 验证：测试覆盖 9 月订单排除、取消订单排除、未关闭订单处理、Full 专用端点、账单去重/未知类型、物流字段缺失、人工物流成本保留及旧版快照归档。

### 2026-09-08 合并包裹物流费与人工成本继承修复

- 同一 Mercado Libre 包裹可能包含多笔订单并共享一个 `shipment_id`。物流费现在按 `shipment_id` 只读取一次，再按包裹内 SKU 件数分摊，避免按订单重复扣费。
- 墨西哥本土3店 2026-08 官方净物流费为 60,245.70 MXN；生产重算为 60,245.69 MXN，0.01 MXN 为逐 SKU 两位小数分摊差。
- 月度重建保留头程/海外仓成本时，先把飞书可能返回的富文本数组/对象统一转换为可见文本，再按 SKU 匹配；防止重跑后人工成本被静默清空。
- 回归测试覆盖同包裹多订单与富文本 SKU 两种真实返回形态；线上健康标记为 `ml_shipping_package_dedupe_20260908`、`ml_month_logistics_sku_cell_normalized_20260908`。
...
- 财务点击确认时，系统先逐格回读验证主表、数据源、检查三张 Sheet，再写完成标记和终稿状态；成功卡片直接链接新月报。
- 同月份用 SQLite 持久占位防止多进程重复建表。数据和 ERP 映射未变化时重复触发返回原报表；发生补数或映射变化时，在原生成器报表内刷新，不另建同名副本。
- 月结卡片动作也写入 SQLite 有序持久台账：已成功动作不会因旧卡延迟投递而再次执行；异常动作会立即释放为可重试；同月并发选择只允许最后领取的有效动作写状态。过期卡在登记动作前先拦截，不会反过来抢占当前有效操作。“处理中”与“已完成”分开反馈。飞书最后动作 key 只用于审计展示，不用于判断并发先后。
- 成本重算逐行写入期间有独立的 SQLite 持久标记，运营/财务确认会暂缓；退回仍可登记为更新决定，使旧重算完成后不能改月结状态或发结果卡。
- 产品库优先使用独立的产品数据 App 凭据；如生产服务未单独配置，可复用已有且已验证拥有产品库只读权限的 `FEISHU_APP_ID/SECRET`，无需重复增加生产密钥。
- 生成期间若出现广告抓取失败或退回操作，会先发布跨进程取消标记，禁止把该次生成写成财务终稿。

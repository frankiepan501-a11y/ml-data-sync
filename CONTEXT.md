...
Feishu report generation must also follow `docs/repairs/2026-09-14-feishu-numeric-readback.md`: numeric-looking strings are not valid financial cells, source read-back covers all 52 columns, evaluated formula read-back covers every formula column, and V4 approval state must not inherit V3 confirmation.

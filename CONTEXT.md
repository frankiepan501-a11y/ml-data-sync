...
A seller/month advertising refresh is valid only when the item-level totals match Mercado Libre `metrics_summary` within rounding tolerance. Fresh Feishu rows are verified before old rows are removed, then the final seller/month set is read back. A manual production repair must call monthly-close re-audit as a separate final step so ordinary per-seller syncs do not emit premature or repeated cards.
